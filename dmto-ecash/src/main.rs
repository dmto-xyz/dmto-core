...
fn main() -> Result<()> {
    ...
    let blind_sigs_with_proof = mint.swap(alice.notes.clone(), blinded_outputs)?;
    println!("Swap successful, mint reissued notes");

    // Bob unblinds and stores new notes
    let values = [4u64, 2u64];
    for (i, &value) in values.iter().enumerate() {
        let key = mint.keys.get(&value).expect("denomination exists");
        ...
        assert!(
            ...
            "DLEQ verification failed for {value} unit note!"
        );
        println!("DLEQ proof verified for {value} unit note");
    ...
    // Keep a copy of a note to demonstrate double-spend rejection at the mint.
    let reused = bob.notes[0].clone();

    // Bob spends
    bob.spend(&mint, 6)?;
    println!("Bob spent 6 units");

    // Double-spend attempt: re-submit an already-spent note to the mint.
    println!("Attempting to reuse an already-spent note...");
    match mint.verify_and_spend(&reused) {
        Err(e) => println!("Rejected as expected: {e}"),
        Ok(()) => panic!("double spend was NOT detected!"),
    }
